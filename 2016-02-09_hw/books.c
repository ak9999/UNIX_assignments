...
int main(int argc, char ** argv)
{
    ...
    /*
     * ssize_t num_bytes;
     * ssize_t is just a signed int, so just use int.
     */
    ...
    int read_bytes = read(file_desc, buffer, num_bytes);
    printf("Reading %zd bytes.\n", read_bytes);
	printf("Reading --->%s<---.\n", buffer);

	// Output the contents of buffer onto terminal.
	int written_bytes = write(1, buffer, read_bytes);
...
int getBytePos(int row, int fd)
{
	...
	lseek(fd, 0, SEEK_SET);
	int read_bytes = read(fd, buf, num_bytes);
